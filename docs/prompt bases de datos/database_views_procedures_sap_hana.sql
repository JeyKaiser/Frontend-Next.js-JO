...
-- Trigger TRG_REFERENCIAS_UPDATE (omitido): actualizaba FECHA_MODIFICACION en T_REFERENCIAS.
-- Trigger TRG_TRAZABILIDAD_TIEMPO (omitido): calculaba TIEMPO_PERMANENCIA en T_TRAZABILIDAD para 'ENTREGADO'.
...
-- Elimina la función si existe previamente para evitar conflictos
DROP FUNCTION FN_DIAS_HABILES;

CREATE FUNCTION FN_DIAS_HABILES (
    FECHA_INICIO DATE,
    FECHA_FIN    DATE
)
RETURNS INTEGER
...
AS
DECLARE v_dias_habiles INTEGER;
BEGIN
    SELECT COUNT(*)
      INTO v_dias_habiles
      FROM SERIES_GENERATE_DATE(:FECHA_INICIO, :FECHA_FIN, INTERVAL '1' DAY) AS T (FECHA)
     WHERE WEEKDAY(T.FECHA) NOT IN (5,6);
...
-- Índice para búsquedas de trazabilidad completa
